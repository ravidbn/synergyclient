...
if [ "$(echo "$UBUNTU_VERSION >= 22.04" | bc -l 2>/dev/null || echo 0)" -eq 1 ]; then
    ...
    sudo apt install -y git zip unzip openjdk-8-jdk python3-pip python3-venv python3-dev python3-setuptools autoconf libtool pkg-config zlib1g-dev libncurses5-dev libncursesw5-dev libncurses6 cmake libffi-dev libssl-dev build-essential
else
    echo "Installing packages for Ubuntu 20.04 and earlier..."
    sudo apt install -y git zip unzip openjdk-8-jdk python3-pip python3-venv python3-dev python3-setuptools python3-distutils autoconf libtool pkg-config zlib1g-dev libncurses5-dev libncursesw5-dev libtinfo5 cmake libffi-dev libssl-dev build-essential
fi
